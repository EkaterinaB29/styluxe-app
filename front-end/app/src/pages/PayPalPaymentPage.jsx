...
const PayPalPaymentPage = () => {
  ...
  const getServiceDetails = (serviceId) => {
    const services = {
      1: {
        title: "General living space advices",
        description: "Up to 5 hours meetings",
        price: 29,
      },
      2: {
        title: "Complete room redesign ",
        description: "Interior and exterior works",
        price: 39,
      },
      3: {
        title: "Unique Furniture for living",
        description: "Complete apartment remodelling",
        price: 59,
      },
    };
    ...
    if (!serviceId || !professionalId || !serviceDetails) {
      ...
      setAlerts(
        <div className="ms-alert ms-action2 ms-small">
          ...
          <p>
            Missing service or professional data. Please go back and select a
            service.
          </p>
        </div>
      );
    ...
    const loadPayPalSdk = () => {
      return new Promise((resolve, reject) => {
        const existingScript = document.querySelector(
          `script[src^="https://www.paypal.com/sdk/js?client-id=AeOXxe44mQC4gNOXKspUb5MbGxAE_1nbHwwahivZNYoLBnUhFjEAvTqwbFM--ltkn2L_HVA6-XcL7dKf"]`
        );
        if (existingScript) {
          resolve();
          return;
          ...
          window.paypal
            .Buttons({
              ...
              createOrder: async () => {
                try {
                  ...
                  const response = await fetch(
                    "http://88.200.63.148:8211/api/orders/create_order",
                    {
                      method: "POST",
                      headers: {
              ...
              onApprove: async (data) => {
                try {
                  const response = await fetch(
                    "http://88.200.63.148:8211/api/orders/complete_order",
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json; charset=utf-8",
                  ...
                  setAlerts(
                    <div className="ms-alert ms-action2 ms-small">
                      <span className="ms-close"></span>
                      <p>Failed to capture order. Please try again.</p>
                    </div>
                  );
                }
              },
