...
const UserView = () => {
    ...
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        const userResponse = await axios.get(
          `http://88.200.63.148:8211/api/user/profile/${professionalId}`,
          config
        );
        ...
        const postsResponse = await axios.get(
          `http://88.200.63.148:8211/api/posts/user/${professionalId}`
        );
        setPosts(postsResponse.data);

        if (userResponse.data.role === "Professional") {
          const reviewsResponse = await axios.get(
            `http://88.200.63.148:8211/api/reviews/professional/${professionalId}`,
            config
          );
  ...
  const portfolioUrl =
    user.role === "Professional"
      ? `http://88.200.63.148:8211${user.portfolio[0].file_path}`
      : null;
      ...
      <div className="user-view">
        ...
        {user.role === "Professional" && (
          <div>
            <div className="bio">
              <h2>Education:</h2>
              <p>{user.portfolio[0].education_history}</p>
            </div>
                      ...
                      <div className="bar-chart">
                        <div className="bar">
                          ...
                          <div
                            className="bar-fill"
                            style={{
                              width: `${(review.quality_of_work / 5) * 100}%`,
                            }}
                          ></div>
                        </div>
                        <div className="bar">
                        ...
                        <div className="bar">
                          ...
                          <div
                            className="bar-fill"
                            style={{
                              width: `${(review.reliability / 5) * 100}%`,
                            }}
                          ></div>
                        </div>
                        <div className="bar">
                          ...
                          <div
                            className="bar-fill"
                            style={{
                              width: `${(review.satisfaction / 5) * 100}%`,
                            }}
                          ></div>
                        </div>
                      </div>
