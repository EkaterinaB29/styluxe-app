...
const SinglePostContent = ({ post }) => {
  ...
  const handleDelete = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.delete(`http://88.200.63.148:8211/api/posts/${postId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleSave = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`http://88.200.63.148:8211/api/posts/${postId}`, {
        content: editContent,
      }, {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    <div className="single-post-content">
      ...
      {isAuthor ? (
        ...
        <div className="notification">
          <p>You cannot edit or delete this post because you are not the author.</p>
        </div>
      )}
    </div>
  );
