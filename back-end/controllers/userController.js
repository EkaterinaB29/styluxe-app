...
const registerClient = asyncHandler(async (req, res) => {
  try {
    const { first_name, last_name, location, birthday, email, password } =
      req.body;
    ...
    res
      .status(201)
      .json({ message: "Client registered successfully", token, userId });
  } catch (err) {
    if (err.code === "ER_DUP_ENTRY") {
      return res
        .status(400)
        .json({ message: "Email already registered", code: "ER_DUP_ENTRY" });
    }
  ...
  try {
    ...
    if (portfolioFile) {
      ...
      try {
        ...
        return res
          .status(500)
          .json({ error: "Failed to create portfolio", details: portfolioErr });
      }
    } else {
      res.status(201).json({
        message: "Professional registered successfully",
...
const updateProfessionalProfile = asyncHandler(async (req, res) => {
  ...
  const portfolio_file =
    req.files && req.files["portfolio"]
      ? `/uploads/${req.files["portfolio"][0].filename}`
      : undefined;
